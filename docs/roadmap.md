...
proof gates. The separate, isolated PC-sampling preflight initially returned
`CUPTI_ERROR_INSUFFICIENT_PRIVILEGES`. An authorized administrative rerun now passes
actual data collection and cleanup, with schema-valid `ready`/exit `0` and no residual
diagnostic worker; the medium-token control still fails permission checks. This
removes the sampling permission blocker, not any execution proof gate. No driver,
registry, TDR or NVIDIA setting was changed by the diagnostic. Other memory/order/completeness and
