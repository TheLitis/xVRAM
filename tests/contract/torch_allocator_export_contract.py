#!/usr/bin/env python3
"""Enforce the public symbol contract of the PyTorch allocator plugin."""
...
EXPECTED = {
    "xvram_torch_alloc",
    "xvram_torch_free",
    "xvram_torch_get_last_error",
    "xvram_torch_get_stats",
    "xvram_torch_reset_stats",
}
ELF_VERSION_NODE = "XVRAM_TORCH_ALLOCATOR_1.0"
...
def _self_test() -> int:
    ...
    parsed = _parse_elf_exports("\n".join(lines))
    errors = _validate_elf(*parsed)
    if errors:
    ...
    exports, nodes, versions = _parse_elf_exports(
        ...
        f"xvram_torch_alloc@@{ELF_VERSION_NODE} T 100\n"
        "unexpected_absolute A 200\n"
    )
    if exports != {"xvram_torch_alloc", "unexpected_absolute"}:
        print(f"unexpected parser result: {sorted(exports)}")
        return 1
    if nodes != {ELF_VERSION_NODE} or versions != {"xvram_torch_alloc": ELF_VERSION_NODE}:
...
def main() -> int:
    if sys.argv[1:] == ["--self-test"]:
        return _self_test()
    if len(sys.argv) != 2:
        print("usage: torch_allocator_export_contract.py <shared-library>")
        return 64
