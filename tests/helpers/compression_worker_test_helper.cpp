...
using xvram::compression::FinalWorkerPayload;
...
[[nodiscard]] bool emit(const WorkerFrameType type, const std::uint64_t sequence,
  ...
  return xvram::compression::write_worker_frame(std::cout, type, sequence, payload);
}

int success_mode() {
  if (!emit(WorkerFrameType::plan, 1U, R"({"state":"planned"})") ||
      !emit(WorkerFrameType::heartbeat, 2U, "{}") ||
      !emit(WorkerFrameType::progress, 3U, R"({"state":"running"})")) {
    return 74;
  }
  FinalWorkerPayload final;
  final.exit_code = 0;
  final.json = R"({"state":"completed"})";
  final.text = "compression worker completed\n";
  const std::string encoded = xvram::compression::encode_final_worker_payload(final);
...
[[noreturn]] void crash_mode(const std::string& marker_path) {
  if (!write_marker(marker_path) || !emit(WorkerFrameType::plan, 1U, R"({"state":"planned"})") ||
      !emit(WorkerFrameType::progress, 2U, R"({"state":"before_crash"})")) {
    std::_Exit(74);
  }
  std::_Exit(42);
}
...
[[noreturn]] void hang_mode(const std::string& marker_path, const bool emit_trace) {
  ...
  if (emit_trace && !emit(WorkerFrameType::trace, 2U,
                          R"({"report_type":"xvram.compression_trace","sequence":1})"
                          "\n")) {
    std::_Exit(74);
  }
  for (;;) {
...
[[noreturn]] void non_monotonic_mode(const std::string& marker_path) {
  if (!write_marker(marker_path) || !emit(WorkerFrameType::plan, 1U, R"({"state":"planned"})") ||
      !emit(WorkerFrameType::progress, 2U, R"({"state":"running"})") ||
      !emit(WorkerFrameType::heartbeat, 2U, "{}")) {
    std::_Exit(74);
  }
  for (;;) {
...
int truncated_mode() {
  ...
  if (!xvram::compression::write_worker_frame(stream, WorkerFrameType::plan, 1U, "{}")) {
    return 70;
  }
...
int oversized_mode() {
  ...
  header[2] = 'Z';
  header[3] = '1';
  write_u16(header, 4U, xvram::compression::worker_protocol_version);
  write_u16(header, 6U, static_cast<std::uint16_t>(WorkerFrameType::progress));
  write_u32(header, 8U, xvram::compression::maximum_worker_payload_bytes + 1U);
