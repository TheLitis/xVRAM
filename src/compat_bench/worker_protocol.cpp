...
constexpr std::array<std::uint8_t, 4> magic{'X', 'V', 'I', '1'};
...
std::optional<FinalWorkerPayload> decode_final_worker_payload(const std::string_view payload,
  ...
  if (payload.size() > maximum_worker_payload_bytes) {
    error = "final compatibility worker payload exceeds the frame limit";
    return std::nullopt;
  }
  if (payload.size() < 12U) {
    error = "final compatibility worker payload is truncated";
    return std::nullopt;
  }
  ...
  if (expected_size != payload.size()) {
    error = "final compatibility worker payload lengths are invalid";
    return std::nullopt;
  }
  if (!valid_exit_code(static_cast<std::int32_t>(exit_code))) {
    error = "final compatibility worker payload contains an invalid exit code";
    return std::nullopt;
  }
  if (json_size == 0U) {
    error = "final compatibility worker payload contains an empty report";
    return std::nullopt;
  }
...
bool WorkerFrameDecoder::append(const char* data, const std::size_t size,
  ...
  if (size > 0 && data == nullptr) {
    error = "compatibility worker protocol received a null buffer";
    return false;
  }
  if (size > 0) {
  ...
  for (;;) {
    ...
    if (!std::equal(magic.begin(), magic.end(), buffer_.begin())) {
      error = "compatibility worker protocol magic does not match";
      return false;
    }
    ...
    if (version != worker_protocol_version) {
      error = "compatibility worker protocol version is unsupported";
      return false;
    }
    if (!valid_type(raw_type)) {
      error = "compatibility worker protocol frame type is invalid";
      return false;
    }
    const auto type = static_cast<WorkerFrameType>(raw_type);
    if (payload_size > payload_limit(type)) {
      error = "compatibility worker protocol payload exceeds the frame limit";
      return false;
    }
    ...
    if (sequence == 0 || (have_sequence_ && sequence <= last_sequence_)) {
      error = "compatibility worker protocol sequence did not increase";
      return false;
    }
    if (state_ == State::awaiting_plan && type != WorkerFrameType::plan) {
      error = "compatibility worker protocol must begin with a plan frame";
      return false;
    }
    if (state_ == State::running && type == WorkerFrameType::plan) {
      error = "compatibility worker protocol contains a duplicate plan frame";
      return false;
    }
    if (state_ == State::finished) {
      error = "compatibility worker protocol contains data after the final frame";
      return false;
    }
...
bool WorkerFrameDecoder::finish(std::string& error) const {
  if (!buffer_.empty()) {
    error = "compatibility worker protocol ended with a truncated frame";
    return false;
  }
  if (state_ != State::finished) {
    error = "compatibility worker protocol ended before the final frame";
    return false;
  }
  return true;
}
