  ...
  [[nodiscard]] bool consume(const char* data, const std::size_t size, bool& made_progress,
    ...
    for (auto& frame : frames) {
      ...
      if (frame.type == WorkerFrameType::progress) {
        ...
        const auto parsed = std::from_chars(frame.payload.data(),
                                            frame.payload.data() + frame.payload.size(), progress);
        if (parsed.ec != std::errc{} || parsed.ptr != frame.payload.data() + frame.payload.size() ||
            progress <= last_progress_) {
          error = "compatibility worker progress counter did not strictly increase";
          return false;
        }
      ...
      if (frame.type == WorkerFrameType::trace) {
        ...
        try {
          ...
          accepted = false;
          error = "compatibility trace sink threw while delivering a batch";
        }
        if (!accepted) {
          trace_error_ = true;
          if (error.empty()) {
            error = "compatibility trace sink rejected a batch";
          }
          return false;
        }
        ++trace_frames_;
      ...
      if (final_.has_value()) {
        error = "compatibility worker protocol sent more than one final frame";
        return false;
      }
...
WorkerControllerResult run_isolated_worker(const WorkerControllerOptions& options) {
  ...
  if (!result.timed_out && !result.protocol_error && result.started && !result.final.has_value()) {
    ...
    result.error = "compatibility worker exited without a final protocol frame";
  }
  return result;
}
