...
constexpr std::array<std::uint8_t, 4> magic{'X', 'V', 'G', '1'};
...
bool write_worker_frame(std::ostream& output, const WorkerFrameType type,
  ...
  if (!valid_type(static_cast<std::uint16_t>(type)) || sequence == 0 ||
      payload.size() > maximum_worker_payload_bytes ||
...
std::optional<FinalWorkerPayload> decode_final_worker_payload(const std::string_view payload,
  ...
  if (payload.size() > maximum_worker_payload_bytes) {
    error = "final GEMM worker payload exceeds the frame limit";
    return std::nullopt;
  }
  if (payload.size() < 12U) {
    error = "final GEMM worker payload is truncated";
    return std::nullopt;
  }
  ...
  if (expected_size != payload.size()) {
    error = "final GEMM worker payload lengths are invalid";
    return std::nullopt;
  }
...
bool WorkerFrameDecoder::append(const char* data, const std::size_t size,
  ...
  if (size > 0 && data == nullptr) {
    error = "GEMM worker protocol received a null buffer";
    return false;
  }
  if (size > 0) {
  ...
  for (;;) {
    ...
    if (!std::equal(magic.begin(), magic.end(), buffer_.begin())) {
      error = "GEMM worker protocol magic does not match";
      return false;
    }
    ...
    if (version != worker_protocol_version) {
      error = "GEMM worker protocol version is unsupported";
      return false;
    }
    if (!valid_type(type)) {
      error = "GEMM worker protocol frame type is invalid";
      return false;
    }
    if (payload_size > maximum_worker_payload_bytes) {
      error = "GEMM worker protocol payload exceeds the frame limit";
      return false;
    }
    ...
    if (sequence == 0 || (have_sequence_ && sequence <= last_sequence_)) {
      error = "GEMM worker protocol sequence did not increase";
      return false;
    }

    const auto frame_type = static_cast<WorkerFrameType>(type);
    if (state_ == State::awaiting_plan && frame_type != WorkerFrameType::plan) {
      error = "GEMM worker protocol must begin with a plan frame";
      return false;
    }
    if (state_ == State::running && frame_type == WorkerFrameType::plan) {
      error = "GEMM worker protocol contains a duplicate plan frame";
      return false;
    }
    if (state_ == State::finished) {
      error = "GEMM worker protocol contains data after the final frame";
      return false;
    }
    ...
    have_sequence_ = true;
    if (frame_type == WorkerFrameType::plan) {
      state_ = State::running;
    } else if (frame_type == WorkerFrameType::final) {
...
bool WorkerFrameDecoder::finish(std::string& error) const {
  if (!buffer_.empty()) {
    error = "GEMM worker protocol ended with a truncated frame";
    return false;
  }
  if (state_ != State::finished) {
    error = "GEMM worker protocol ended before the final frame";
    return false;
  }
  return true;
}
