...
constexpr std::array<std::uint8_t, 4> magic{'X', 'V', 'P', '1'};
...
std::string encode_final_worker_payload(const FinalWorkerPayload& payload) {
  ...
  if (output.size() > maximum_worker_payload_bytes) {
    return {};
  }
  return output;
}
...
std::optional<FinalWorkerPayload> decode_final_worker_payload(const std::string_view payload,
                                                              std::string& error) {
  if (payload.size() < 12U) {
    error = "final worker payload is truncated";
    return std::nullopt;
  }
  ...
  if (expected_size != payload.size()) {
    error = "final worker payload lengths are invalid";
    return std::nullopt;
  }
...
bool WorkerFrameDecoder::append(const char* data, const std::size_t size,
  ...
  if (size > 0 && data == nullptr) {
    error = "worker protocol received a null buffer";
    return false;
  }
  if (size > 0) {
  ...
  for (;;) {
    ...
    if (!std::equal(magic.begin(), magic.end(), buffer_.begin())) {
      error = "worker protocol magic does not match";
      return false;
    }
    ...
    if (!valid_type(type)) {
      error = "worker protocol frame type is invalid";
      return false;
    }
    if (payload_size > maximum_worker_payload_bytes) {
      error = "worker protocol payload exceeds the limit";
      return false;
    }
    ...
    if (have_sequence_ && sequence <= last_sequence_) {
      error = "worker protocol sequence did not increase";
      return false;
    }
    WorkerFrame frame;
    frame.type = static_cast<WorkerFrameType>(type);
...
bool WorkerFrameDecoder::finish(std::string& error) const {
  if (!buffer_.empty()) {
    error = "worker protocol ended with a truncated frame";
    return false;
  }
  return true;
}
