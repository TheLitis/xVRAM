...
constexpr std::array<std::uint8_t, 4> magic{'X', 'V', 'C', '1'};
...
bool write_worker_frame(std::ostream& output, const WorkerFrameType type,
  ...
  const std::size_t type_limit = type == WorkerFrameType::trace
                                     ? maximum_cache_trace_batch_bytes
                                     : maximum_cache_worker_payload_bytes;
  if (payload.size() > type_limit || payload.size() > std::numeric_limits<std::uint32_t>::max()) {
    return false;
  }
  std::string header;
  header.reserve(header_bytes);
  header.append(reinterpret_cast<const char*>(magic.data()), magic.size());
  append_u16(header, cache_worker_protocol_version);
...
std::string encode_final_worker_payload(const FinalWorkerPayload& payload) {
  if (payload.json.size() > std::numeric_limits<std::uint32_t>::max() ||
      payload.text.size() > std::numeric_limits<std::uint32_t>::max()) {
    return {};
  }
  std::string output;
  output.reserve(12U + payload.json.size() + payload.text.size());
  ...
  if (output.size() > maximum_cache_worker_payload_bytes) {
    return {};
  }
  return output;
}
...
std::optional<FinalWorkerPayload> decode_final_worker_payload(const std::string_view payload,
                                                              std::string& error) {
  if (payload.size() < 12U) {
    error = "final cache worker payload is truncated";
    return std::nullopt;
  }
  ...
  if (expected_size != payload.size()) {
    error = "final cache worker payload lengths are invalid";
    return std::nullopt;
  }
...
bool WorkerFrameDecoder::append(const char* data, const std::size_t size,
  ...
  if (size > 0 && data == nullptr) {
    error = "cache worker protocol received a null buffer";
    return false;
  }
  if (size > 0) {
  ...
  for (;;) {
    ...
    if (!std::equal(magic.begin(), magic.end(), buffer_.begin())) {
      error = "cache worker protocol magic does not match";
      return false;
    }
    const std::uint16_t version = read_u16(buffer_.data() + 4U);
    const std::uint16_t type = read_u16(buffer_.data() + 6U);
    const std::uint32_t payload_size = read_u32(buffer_.data() + 8U);
    const std::uint64_t sequence = read_u64(buffer_.data() + 12U);
    if (version != cache_worker_protocol_version) {
      error = "cache worker protocol version is unsupported";
      return false;
    }
    if (!valid_type(type)) {
      error = "cache worker protocol frame type is invalid";
      return false;
    }
    const auto frame_type = static_cast<WorkerFrameType>(type);
    const std::uint32_t type_limit = frame_type == WorkerFrameType::trace
                                         ? maximum_cache_trace_batch_bytes
                                         : maximum_cache_worker_payload_bytes;
    if (payload_size > type_limit) {
      error = "cache worker protocol payload exceeds the frame limit";
      return false;
    }
    ...
    if (have_sequence_ && sequence <= last_sequence_) {
      error = "cache worker protocol sequence did not increase";
      return false;
    }
    WorkerFrame frame;
...
bool WorkerFrameDecoder::finish(std::string& error) const {
  if (!buffer_.empty()) {
    error = "cache worker protocol ended with a truncated frame";
    return false;
  }
  return true;
}
