...
inline constexpr std::uint16_t worker_protocol_version = 1;
inline constexpr std::uint32_t maximum_worker_payload_bytes = 1024U * 1024U;
inline constexpr std::uint32_t maximum_trace_batch_bytes = 64U * 1024U;

enum class WorkerFrameType : std::uint16_t {
  plan = 1,
  heartbeat = 2,
  progress = 3,
  trace = 4,
...
struct WorkerFrame {
  WorkerFrameType type = WorkerFrameType::heartbeat;
...
private:
  enum class State { awaiting_plan, running, finished };

  std::vector<std::uint8_t> buffer_;
  std::uint64_t last_sequence_ = 0;
  bool have_sequence_ = false;
  State state_ = State::awaiting_plan;
};
